...
import { hideValidationError, reConfirmEmailReq } from '../features/userSlice'
    ...
    <div className="flex flex-col items-center justify-center gap-4 pb-12">
      ...
      <div className="w-11/12 md:w-10/12 lg:w-8/12 xl:w-7/12 2xl:w-6/12 flex-col justify-center items-center gap-2 text-center rounded-xl shadow-2xl p-6 py-14 bg-peak mt-10">
        ...
        <Formik
          ...
          onSubmit={(values) => {
            dispatch(reConfirmEmailReq(values.email))
            setNotify(true)
          }}
        >
          {({ errors, touched }) => (
            <Form className="w-full flex flex-col justify-center items-center gap-4 rounded p-2">
              ...
              >
                ...
                <button
                  type="submit"
                  className="text-xl font-bold flex justify-center items-center gap-2 ltr rounded bg-sky text-rock p-3 pb-4 hover-rise cursor-pointer"
                >
                  {pending ? 'جاري الإرسال' : 'تفعيل الحساب'}
                </button>
              </div>
            </Form>
          )}
